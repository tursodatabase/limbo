...
impl WriteInfo {
    fn new() -> WriteInfo {
        WriteInfo {
            ...
            sibling_count: Cell::new(0),
            first_divider_cell: Cell::new(0),
    ...
    fn balance_non_root(&mut self) -> Result<CursorResult<()>> {
        ...
        let (next_write_state, result) = match state {
            ...
            WriteState::BalanceNonRootWaitLoadPages => {
                ...
                let sibling_count = write_info.sibling_count.get();
                let first_divider_cell = write_info.first_divider_cell.get();

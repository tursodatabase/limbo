...
use crate::storage::sqlite3_ondisk::{
    read_btree_cell, read_u32, read_varint, write_varint, BTreeCell, DatabaseHeader, PageContent,
    PageType, TableInteriorCell, TableLeafCell,
};
...
impl BTreeCursor {
    ...
    fn balance(&mut self) -> Result<CursorResult<()>> {
        ...
        match state {
            WriteState::BalanceStart => {
                ...
                self.stack.pop();
                dbg!(self.stack.current_cell_index());
                // We always advance page on traversal so we need to point to correct page again
                self.stack.retreat();
                dbg!(self.stack.current_cell_index());
    ...
    fn balance_non_root(&mut self) -> Result<CursorResult<()>> {
        ...
        let state = self
            ...
            .state
            .clone();
        log::trace!("balance_non_root(state={:?})", state);
        let (next_write_state, result) = match state {
            ...
            WriteState::BalanceNonRootWaitLoadPages => {
                ...
                for i in (1..sibling_count_new).rev() {
                    ...
                    log::trace!("end cell_left={}", cell_left);

...
fn finish_read_database_header(buf: IOBuff, header: Rc<RefCell<DatabaseHeader>>) -> Result<()> {
...
pub fn begin_sync(page_io: Rc<dyn DatabaseStorage>, syncing: Rc<Cell<bool>>) -> Result<()> {
    assert!(!syncing.get());
    syncing.set(true);
